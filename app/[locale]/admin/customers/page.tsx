...
export default async function CustomersAdmin({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div>
      ...
      <h1 className="h-display text-4xl mt-2 mb-6">Customers ({customers.length})</h1>
      <form className="mb-6"><input name="q" defaultValue={sp.q} className="input" placeholder="Search name, phone, email..." /></form>
      <div className="glass overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="border-b border-line text-xs tracking-cinematic text-muted">
            <tr><th className="p-3 text-left">Name</th><th className="p-3 text-left">Phone</th><th className="p-3 text-left">Email</th><th className="p-3 text-left">VIP</th><th className="p-3 text-left">Orders</th><th className="p-3 text-left">Spent</th><th className="p-3 text-left">Joined</th><th></th></tr>
          </thead>
          <tbody>
            {customers.map((c) => (
              <tr key={c.id} className="border-b border-line/40 hover:bg-white/5">
                <td className="p-3">{c.name}{c.blocked && <span className="ml-2 text-blood text-[10px]">BLOCKED</span>}</td>
                ...
                <td className="p-3"><Link href={`/${locale}/admin/customers/${c.id}`} className="text-electric text-xs">VIEW →</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
