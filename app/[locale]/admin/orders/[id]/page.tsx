...
export default async function OrderDetail({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return (
    <div>
      <p className="text-xs tracking-cinematic text-muted">— ORDER · {order.number}</p>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mt-6">
        <div className="lg:col-span-2 glass p-6">
          <h3 className="text-xs tracking-cinematic text-muted mb-4">— ITEMS</h3>
        ...
        <div className="space-y-4">
          <div className="glass p-6">
            <h3 className="text-xs tracking-cinematic text-muted mb-3">— CUSTOMER</h3>
          ...
          {ship && (
            <div className="glass p-6">
              <h3 className="text-xs tracking-cinematic text-muted mb-3">— SHIPPING</h3>
          ...
          <div className="glass p-6">
            <h3 className="text-xs tracking-cinematic text-muted mb-3">— TOTALS</h3>
            <div className="text-sm space-y-1">
              <div className="flex justify-between"><span className="text-muted">Subtotal</span><span>{order.currency} {order.subtotal.toLocaleString()}</span></div>
              <div className="flex justify-between"><span className="text-muted">Shipping</span><span>{order.shipping}</span></div>
              <div className="flex justify-between"><span className="text-muted">Discount</span><span>{order.discount}</span></div>
              <div className="flex justify-between font-display text-lg pt-2 border-t border-line"><span>Total</span><span>{order.currency} {order.total.toLocaleString()}</span></div>
            </div>
          </div>
        </div>
      </div>
