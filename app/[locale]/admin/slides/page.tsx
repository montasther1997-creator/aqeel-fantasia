...
export default async function SlidesAdmin({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params; await requireAdmin(locale);
  const t = await getTranslations('admin.slides');
    ...
    <div className="space-y-10">
      <header className="flex items-end justify-between gap-6 pb-6 border-b border-line">
        <div>
          ...
          <p className="ed-caption text-muted num mt-3">{slides.length}</p>
        </div>
      </header>
      <SlidesManager initial={slides} />
    </div>
  );
}
