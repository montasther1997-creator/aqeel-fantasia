  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass p-6 space-y-4">
        ...
        <textarea placeholder={t('textArPlaceholder')} dir="rtl" className="input min-h-[80px]" value={d.textAr} onChange={(e) => setD({ ...d, textAr: e.target.value })} />
        <textarea placeholder={t('textEnPlaceholder')} className="input min-h-[80px]" value={d.textEn} onChange={(e) => setD({ ...d, textEn: e.target.value })} />
        <button onClick={add} className="btn-primary"><Plus className="w-4 h-4" /> {t('addNote')}</button>
      </div>
...
function NoteRow({ n, products }: { n: any; products: any[] }) {
  ...
  const del = async () => { if (!confirm(tc('confirmDelete'))) return; await fetch(`/api/admin/notes/${n.id}`, { method: 'DELETE' }); router.refresh(); };
  ...
  return (
    <div className="glass p-4 space-y-3">
      <div className="grid grid-cols-12 gap-2 items-center">
        ...
        <label className="col-span-1 text-xs flex items-center gap-1"><input type="checkbox" checked={d.active} onChange={(e) => setD({ ...d, active: e.target.checked })} /> {tc('on')}</label>
      ...
      <button onClick={del} className="text-xs text-blood flex items-center gap-1"><Trash2 className="w-3 h-3" /> {t('delete')}</button>
    </div>
  );
}
