...
export function AdminUserManager({ initial }: { initial: any[] }) {
  ...
  const del = async (id: string) => { if (!confirm('Delete?')) return; await fetch(`/api/admin/users/${id}`, { method: 'DELETE' }); router.refresh(); };
  return (
    <div className="space-y-4">
      <div className="glass p-4 grid grid-cols-12 gap-2">
        <input placeholder="Name" className="input col-span-2" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} />
        <input placeholder="Email" type="email" className="input col-span-3" value={d.email} onChange={(e) => setD({ ...d, email: e.target.value })} />
        <input placeholder="Password" type="password" className="input col-span-3" value={d.password} onChange={(e) => setD({ ...d, password: e.target.value })} />
