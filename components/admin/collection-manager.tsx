...
function Row({ c }: any) {
  ...
  const del = async () => { if (!confirm('Delete?')) return; await fetch(`/api/admin/collections/${c.id}`, { method: 'DELETE' }); router.refresh(); };
  return (
    <div className="grid grid-cols-12 gap-2 p-3 items-center">
      ...
      <span className="col-span-1 text-xs text-muted">{c._count?.products ?? 0} prod</span>
