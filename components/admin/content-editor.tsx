...
function NewKey() {
  ...
  return (
    <div className="glass p-4 grid grid-cols-12 gap-2 items-end">
      <input placeholder={t('keyPlaceholder')} className="input col-span-3" value={d.key} onChange={(e) => setD({ ...d, key: e.target.value })} />
      <input placeholder={t('groupPlaceholder')} className="input col-span-2" value={d.group} onChange={(e) => setD({ ...d, group: e.target.value })} />
      <input placeholder={t('valueEnPlaceholder')} className="input col-span-3" value={d.valueEn} onChange={(e) => setD({ ...d, valueEn: e.target.value })} />
      <input placeholder={t('valueArPlaceholder')} className="input col-span-3" value={d.valueAr} onChange={(e) => setD({ ...d, valueAr: e.target.value })} />
...
function Row({ it }: any) {
  ...
  // Note: delete intentionally not exposed in current UI (no del button); keep refresh after save.
  void tc;
  return (
